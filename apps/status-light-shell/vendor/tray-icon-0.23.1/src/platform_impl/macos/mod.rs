...
impl TrayIcon {
    ...
    fn create(
        ...
        if let Some(menu) = &attrs.menu {
            // SAFETY: `menu.ns_menu()` is owned by the live menu object and remains valid
            // while the tray icon retains the menu for the duration of this assignment.
            unsafe {
        ...
        // SAFETY: the target view is allocated on the main thread, initialized with
        // the current status-item button frame, and immediately attached as a subview.
    ...
    pub fn set_menu(&mut self, menu: Option<Box<dyn menu::ContextMenu>>) {
        if let (Some(ns_status_item), Some(tray_target)) = (&self.ns_status_item, &self.tray_target)
        {
            // SAFETY: the retained `NSMenu` instance outlives this assignment and Cocoa
            // expects the status item itself to act as the delegate while attached.
            unsafe {
                let menu = menu
                    .as_ref()
    ...
    pub fn show_menu(&self) {
        if let Some(ns_status_item) = &self.ns_status_item {
            // SAFETY: we are on the main thread and forwarding the click to the existing
            // status-item button instance managed by Cocoa.
            unsafe {
                let button = ns_status_item.button(self.mtm).unwrap();
...
define_class!(
    ...
    impl TrayTarget {
        ...
        fn update_tracking_areas(&self) {
            // SAFETY: Cocoa requires tracking areas to be replaced from the view instance
            // itself, and calling the superclass implementation keeps internal state in sync.
            unsafe {
...
fn on_tray_click(this: &TrayTarget, button: MouseButton) {
    ...
    // SAFETY: the tray target is a live subview of the status-item button, so the
    // retained status item and its button are valid for the duration of this callback.
    unsafe {
